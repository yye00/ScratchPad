...
void print_array(const std::complex<double>* array, int size, const std::string& name) {
...
void batch_matrix_multiply(const py::list& A_list, py::array_t<std::complex<double>> B, const py::list& B_shapes_list) {
    ...
    for (size_t idx = 0; idx < A_list.size(); ++idx) {
        ...
        std::vector<const std::complex<double>*> a_array(batch_size);
        std::vector<const std::complex<double>*> b_array(batch_size);
        std::vector<std::complex<double>*> c_array(batch_size);

        for (int i = 0; i < batch_size; ++i) {
            a_array[i] = static_cast<std::complex<double>*>(bufA.ptr);
        ...
        print_array(current_B, total_elements_B, "current_B before cblas_zgemm_batch");
        print_array(next_B, total_elements_B, "next_B before cblas_zgemm_batch");
        ...
        print_array(current_B, total_elements_B, "current_B after cblas_zgemm_batch");
        print_array(next_B, total_elements_B, "next_B after cblas_zgemm_batch");
    }
